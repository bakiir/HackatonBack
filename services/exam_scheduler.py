...
class ExamScheduler:
    ...
    def assign_proctors(self, proctors_path=None):
        ...
        # Подсчет экзаменов для каждой школы
        ...
        for _, row in self.schedule_df.iterrows():
            ...
            for faculty in faculties:
                num_proctors = 4 if '107' in row['Room'] else (2 if row.get('two_rooms_needed', False) else 1)
        ...
        for faculty, proctors in self.faculty_proctors.items():
            ...
            logging.info(f"{faculty}: {len(valid_proctors)} прокторов, {faculty_exams[faculty]} мест для прокторов")

        # Инициализируем словари для отслеживания нагрузки и расписания
        sct_proctor_load = {proctor: 0 for proctor in sct_proctors}
        non_sct_proctor_load = {proctor: 0 for proctor in non_sct_proctors}
        proctor_schedule = {}  # (Date, Time_Slot) -> [proctors]
        ...
        for _, row in self.schedule_df.iterrows():
            ...
            # Определяем количество прокторов
            if '107' in room:
                num_proctors = 4
                logging.info(f"Для {section_id} в аудитории {room} требуется 4 проктора")
            elif two_rooms_needed:
                num_proctors = 2
                logging.info(f"Для {section_id} с two_rooms_needed=True требуется 2 проктора")
            else:
                num_proctors = 1
            ...
            # Выбираем пул прокторов
            if is_sct_subject:
                proctor_pool = sct_proctors
                proctor_load = sct_proctor_load
                logging.info(f"Для {section_id} (ШЦТ) выбираем прокторов из ШЦТ")
            else:
                excluded_faculties = subject_faculties | {'Школа цифровых технологий'}
                proctor_pool = [
                    p for p in non_sct_proctors
                    if not any(p in self.faculty_proctors.get(faculty, []) for faculty in excluded_faculties)
                ]
                proctor_load = non_sct_proctor_load
                logging.info(f"Для {section_id} исключаем прокторов из {excluded_faculties}")

            if not proctor_pool:
                logging.error(
                    f"Нет доступных прокторов для {section_id}: предмет {subject}, исключены {excluded_faculties}")
                raise ValueError(f"Нет доступных прокторов для {section_id}")

            # Фильтруем прокторов: нагрузка ниже максимума и не заняты в этом слоте
            slot_key = (exam_date, time_slot)
            busy_proctors = proctor_schedule.get(slot_key, [])
            available = [
                p for p in proctor_pool
                if proctor_load[p] < MAX_PROCTOR_LOAD and p not in busy_proctors
            ]

            if len(available) < num_proctors:
                logging.error(
                    f"Недостаточно свободных прокторов для {section_id}: требуется {num_proctors}, доступно {len(available)}")
                raise ValueError(f"Недостаточно свободных прокторов для {section_id}")

            # Выбираем прокторов с минимальной нагрузкой
            assigned = []
            for _ in range(num_proctors):
                if not available:
                    logging.error(
                        f"Не удалось найти достаточно прокторов для {section_id}: требуется {num_proctors}, выбрано {len(assigned)}")
                    raise ValueError(f"Недостаточно прокторов для {section_id}")
                # Выбираем проктора с минимальной нагрузкой
                min_load_proctor = min(available, key=lambda p: proctor_load[p])
                assigned.append(min_load_proctor)
                available.remove(min_load_proctor)  # Удаляем из доступных для этой секции

            # Обновляем нагрузку и расписание
            for proctor in assigned:
                proctor_load[proctor] += 1
                if proctor_load[proctor] >= MAX_PROCTOR_LOAD:
                    logging.warning(f"Проктор {proctor} достиг максимальной нагрузки {MAX_PROCTOR_LOAD}")
                proctor_schedule.setdefault(slot_key, []).append(proctor)

            # Формируем строку для столбца Proctor
        ...
        # Логируем статистику нагрузки
        sct_loads = [v for v in sct_proctor_load.values() if v > 0]
        non_sct_loads = [v for v in non_sct_proctor_load.values() if v > 0]
        logging.info(f"Средняя нагрузка на проктора ШЦТ: {statistics.mean(sct_loads) if sct_loads else 0:.2f}")
        logging.info(
            f"Средняя нагрузка на проктора вне ШЦТ: {statistics.mean(non_sct_loads) if non_sct_loads else 0:.2f}")
        logging.info(f"Максимальная нагрузка ШЦТ: {max(sct_loads) if sct_loads else 0}")
        logging.info(f"Максимальная нагрузка вне ШЦТ: {max(non_sct_loads) if non_sct_loads else 0}")

        # Статистика по всем школам
        logging.info("Нагрузка прокторов по школам:")
        for faculty, proctors in self.faculty_proctors.items():
            faculty_loads = [
                sct_proctor_load.get(p, non_sct_proctor_load.get(p, 0))
                for p in proctors if p in available_proctors
            ]
            if faculty_loads:
                logging.info(f"{faculty}: средняя нагрузка {statistics.mean(faculty_loads):.2f}, "
                             f"мин {min(faculty_loads)}, макс {max(faculty_loads)}, "
                             f"{len([p for p in proctors if p in available_proctors])} прокторов, "
                             f"{faculty_exams[faculty]} мест для прокторов")

        # Сохраняем в DataFrame и объект
        self.schedule_df['Proctor'] = assigned_proctors
        self.section_proctors = section_proctors

        # Логируем нагрузку и расписание прокторов для отладки
        logging.info(f"Нагрузка прокторов ШЦТ: {sct_proctor_load}")
        logging.info(f"Нагрузка прокторов вне ШЦТ: {non_sct_proctor_load}")
        logging.info(f"Расписание прокторов: {proctor_schedule}")
    ...
    def create_schedule(self):
        logging.info("Начало создания расписания с группировкой по предметам")
        self.schedule = []
        ...
        try:
            ...
            # 3. НОВАЯ ЛОГИКА: Группировка по предметам
            subjects_groups = {}
            for _, group in exam_groups.iterrows():
                subject = group['Subject']
                if subject not in subjects_groups:
                    subjects_groups[subject] = []
                subjects_groups[subject].append(group)

            # Сортируем предметы по общему количеству студентов (самые большие предметы первыми)
            def get_subject_total_students(subject):
                total = 0
                for group in subjects_groups[subject]:
                    section_students = self.exams_df[self.exams_df['Section'] == group['Section']]['fake_id'].unique()
                    total += len(section_students)
                return total

            sorted_subjects = sorted(subjects_groups.keys(),
                                     key=get_subject_total_students,
                                     reverse=True)

            logging.info(f"Найдено {len(sorted_subjects)} уникальных предметов для планирования")

            # 4. Учёт нагрузки по дням и конфликтов студентов
            day_load = {day.strftime('%Y-%m-%d'): 0 for day in self.custom_dates}
            student_exams_per_day = defaultdict(lambda: defaultdict(int))

            # 5. Планирование по предметам
            current_day_index = 0  # Индекс текущего дня для размещения

            for subject in sorted_subjects:
                logging.info(f"Планирование экзаменов по предмету: {subject}")
                subject_groups = subjects_groups[subject]

                # Сортируем группы внутри предмета по количеству студентов и степени конфликтов
                subject_groups_df = pd.DataFrame(subject_groups)
                subject_groups_df['student_count'] = subject_groups_df['Section'].map(
                    lambda x: len(self.exams_df[self.exams_df['Section'] == x])
                )

                # Вычисляем степень конфликтов для групп этого предмета
                ...
                subject_groups_df['conflict_degree'] = subject_groups_df['Section'].map(section_conflicts)
                subject_groups_df = subject_groups_df.sort_values(by=['conflict_degree', 'student_count'],
                                                                  ascending=False)

                # Планируем группы этого предмета подряд, начиная с текущего дня
                subject_failed = []

                for _, group in subject_groups_df.iterrows():
                    section = group['Section']
                    ...
                    # Пробуем разместить в текущие доступные дни, начиная с current_day_index
                    for day_offset in range(len(self.custom_dates)):
                        day_idx = (current_day_index + day_offset) % len(self.custom_dates)
                        day = self.custom_dates[day_idx]
                        exam_date = day.strftime('%Y-%m-%d')

                        # Проверяем конфликты студентов на этот день
                        day_conflicts = sum(1 for student in students if student_exams_per_day[student][exam_date] >= 1)

                        # Если есть жесткие конфликты (студент уже имеет экзамен в этот день), пробуем следующий день
                        if day_conflicts > 0:
                            continue

                        # Пробуем временные слоты
                        for slot in self.time_slots:
                            ...
                            # Проверяем временные конфликты у студентов
                            time_conflicts = 0
                            ...
                            if two_rooms_needed:
                                ...
                                for room1, room2 in room_pairs:
                                    if self.room_capacities[room1] + self.room_capacities[room2] >= num_students:
                                        suitable_room = f"{room1},{room2}"
                                        break
                            else:
                                ...
                                if suitable_rooms:
                                    suitable_room = suitable_rooms[0]
                            ...
                            if suitable_room:
                                ...
                                # Отмечаем аудитории как занятые
                                if ',' in suitable_room:
                                    room1, room2 = suitable_room.split(',')
                                    self.room_usage[day][slot].add(room1)
                                    self.room_usage[day][slot].add(room2)
                                else:
                                    self.room_usage[day][slot].add(suitable_room)

                                # Обновляем информацию о студентах
                                for student in students:
                                ...
                                # Если это первый экзамен предмета в новом дне, обновляем current_day_index
                                if day_idx > current_day_index:
                                    current_day_index = day_idx

                                logging.info(
                                    f"Запланирован экзамен {section} на {exam_date} {actual_slot} в {suitable_room}")
                                break
                ...
                failed_sections.extend(subject_failed)

                # После планирования всех групп предмета, переходим к следующему дню для следующего предмета
                # чтобы обеспечить разделение предметов по дням
                current_day_index = (current_day_index + 1) % len(self.custom_dates)

                logging.info(
                    f"Завершено планирование предмета {subject}. Запланировано: {len(subject_groups) - len(subject_failed)} из {len(subject_groups)} групп")

            # 6. Второй проход для незапланированных секций (с ослабленными ограничениями)
            if failed_sections:
                logging.info(f"Второй проход для {len(failed_sections)} незапланированных секций")
                ...
                for failed in remaining_sections:
                    ...
                    for day in self.custom_dates:
                        ...
                        for slot in self.time_slots:
                            ...
                            # Во втором проходе разрешаем конфликты студентов
                            conflicts = 0
                            for student in students:
                                if student in self.student_exams:
                                    for exam in self.student_exams[student]:
            ...
            # Вывод статистики по предметам
            if self.schedule_df is not None and not self.schedule_df.empty:
                subject_stats = self.schedule_df.groupby(['Subject', 'Date']).size().reset_index(name='count')
                logging.info("Распределение экзаменов по предметам и дням:")
                for subject in subject_stats['Subject'].unique():
                    subject_dates = subject_stats[subject_stats['Subject'] == subject]
                    dates_info = []
                    for _, row in subject_dates.iterrows():
                        dates_info.append(f"{row['Date']}({row['count']} групп)")
                    logging.info(f"  {subject}: {', '.join(dates_info)}")
